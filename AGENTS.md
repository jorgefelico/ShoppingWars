...
**Shopping Wars** is a first-person game prototype built with **Godot 4.7** and **C#** (Godot.NET.Sdk 4.7.1, .NET 8). The player is in an enclosed arena containing a produce table full of apples; they can look around, pick up products (E), switch between 5 inventory slots (1–5 / mouse wheel), and throw them (LMB). It is a single-scene prototype: no save system, no enemies, no HUD beyond a crosshair and a 5-slot inventory hotbar, no scene transitions.
...
- **`PlayerController.cs`** (`CharacterBody3D`, on `Scenes/player.tscn`) — all gameplay logic.
  - Mouse look: `Head` (yaw) + `Camera` (pitch, clamped ±85°); mouse captured on `_Ready`, released on `ui_cancel` (Esc) — quits if already released.
  - Movement: `Input.GetVector("move_left","move_right","move_back","move_forward")` relative to `Head.GlobalBasis`, sprint (`sprint`) multiplier, custom gravity integration, `MoveAndSlide()`. Tuning constants are `const` at the top of the class.
  ...
  - Pickup (`interact`): if holding → drop in place; else if raycast hit is a `Product` within `PickUpRange` (2 m) and inventory not full → `Freeze = true`, `Reparent(ItemHand)`, `Position = Zero`, `Inventory.AddItem()`.
  - Throw (`fire`): reparent to `GetTree().CurrentScene`, `Freeze = false`, `LinearVelocity = dir * ThrowVelocity` (50) toward a point 10 m in front of the camera.
  - Slot switching: `slot1`–`slot5` and `scroll_up`/`scroll_down` → `SwitchInventorySlot()`.
- **`Inventory.cs`** (`Node`, child of the player) — item-slot logic + the hotbar sync hook. Fixed `Product[]` (default 5, `[Export] InventorySize`), `selectedItemIndex` (public field), `AddItem`/`GetItem`/`SelectNextItem`/`SelectPreviousItem`/`SetCurrentSelectedItem`/`RemoveCurrentSelectedItem`/`IsInventoryFull`. Holds `[Export] InventoryBar`; **every mutator calls `InventoryBar.Refresh(InventoryItems, selectedItemIndex)`**, so the bar stays in sync no matter who mutates the inventory. **No signals.**
- **`Product.cs`** (`RigidBody3D`) — any throwable item. `[Export]` `DisplayName`, `Price`, `Damage`, `ScaleVariation` (random scale 1.0–1.15 in `_Ready`), `Icon` (hotbar texture; `Apple`: `Icons/appleicon.png`). `_Ready` clones the first `MeshInstance3D` in the subtree into a hidden child `_outline` whose surfaces override with `Shaders/outline.gdshader`; `OutlineOn()/OutlineOff()` toggle it.
- **`InventoryBar.cs`** (`CanvasLayer`, child of the player) — bottom hotbar UI. `Refresh(Product[] products, int selectedIndex)` restyles the five slot panels (gold 3px border on the selected slot, black 1px on the rest) and fills each slot's `Label`/`TextureRect` (`Icon` if set, else `DisplayName`; slot number when empty). All slots share one `StyleBoxFlat` subresource in the scene — `Refresh` duplicates it per panel, so never mutate the result of `GetThemeStylebox("panel")` in place.

**Item data flow:** apples (`Prefabs/Products/Apple.tscn`, Jolt rigid bodies: mass 0.2, sphere collider, CCD, `collision_mask = 3`) sit on `Prefabs/produce_table.tscn` → raycast highlights them → pickup freezes + reparents under `Head/Camera/ItemHand` and stores the reference in `Inventory` → slot switching only toggles `Visible` (**all held items remain parented under `ItemHand`, stacked at the same position**) → throw/drop reparents to the current scene, unfreezes, resumes simulation; pickable again. Every mutation (pickup, drop, throw, slot switch) also refreshes the hotbar via `InventoryBar.Refresh`.

**Scene graph** (`Scenes/world.tscn`): `World` → light, `Floor` (80×80 plane), `Player` (instance of `player.tscn` at the origin), `ProduceTable` (table + ~68 `Apple` instances), `Exterior Walls` (40 `PrototypeWall` instances forming an 80 m perimeter). `player.tscn` root `Player` → `CollisionShape3D`, `Head` → `Camera` → `RayCast3D` + `ItemHand`, `CrossHair` (3×3 px `ColorRect`), `Inventory`, `InventoryBar` (CanvasLayer → `HBoxContainer` bottom-center → `Slot1`–`Slot5`, 40×40 `Panel`s each with a `Label` + `TextureRect`, `mouse_filter = 2` throughout).
...
- **Tuning values:** gameplay tuning as `const` in the class (e.g. `Accel = 30f`, `Gravity = 9.8f`); per-instance tuning as `[Export]` with defaults (e.g. `PickUpRange = 2.0f`), overridden in the `.tscn`.
...
2. Core loop to verify: mouse capture works (Esc releases, click re-captures) → aim at an apple (gold outline appears) → E picks it up (appears in hand; its icon appears in the hotbar) → 1–5 / wheel switch slots (only the selected item visible; the hotbar's gold border follows) → LMB throws it toward the crosshair, E drops it in place (icon clears, slot number returns) → thrown/dropped apples fall under Jolt and can be picked up again.
...
- `Product.Price` and `Product.Damage` are exported and set (`Apple`: 3 / 5) but **referenced nowhere** — placeholders for future economy/combat systems; nothing consumes them yet.
- An inventory hotbar exists (5 slots, per-product icons, gold highlight on the selected slot) but there is no other HUD: no money or score display, no save/load, no enemy AI, no scene transitions — building any of these is greenfield work.
